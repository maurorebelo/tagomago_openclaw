...
- **Never post to Twitter/X.** xurl is **read-only** here: `whoami`, `timeline` (optional for “last tweet” checks), and `GET /2/users/{id}/tweets` (live sync — **only your tweets**). Do **not** run `xurl post` or any write. (Revoke app: X → Settings → Security and account access → Apps and sessions.)
...
- **xurl read-only wrapper:** `scripts/xurl-readonly.sh` allows `whoami`, `timeline`, and paths matching `/2/users/[0-9]+/tweets`; logs to `XURL_AUDIT_LOG` (default `/data/.xurl-audit.log`); blocks `post` and other APIs. **Installed on VPS:** `/data/bin/xurl` = wrapper, `/data/bin/xurl-real` = real binary. `run-live-x-nostr-sync.sh` sets `PATH=/data/bin:$PATH`. For the **agent**, the OpenClaw exec config must prepend `/data/bin` **before** Linuxbrew, i.e. `tools.exec.pathPrepend: ["/data/bin", "/data/linuxbrew/.linuxbrew/bin"]`. If `/data/linuxbrew/.linuxbrew/bin` comes first, the agent can hit the real `xurl` and bypass the wrapper and audit log. After updating the script on the host, copy to `/data/bin/xurl` in the container. Audit: `cat /data/.xurl-audit.log`.
...
- **Exec PATH order (critical):** em `/data/.openclaw/openclaw.json`, usar `tools.exec.pathPrepend: ["/data/bin", "/data/linuxbrew/.linuxbrew/bin"]`. `/data/bin` tem de vir primeiro para forçar o wrapper do `xurl`; se Linuxbrew vier antes, o agente pode contornar o bloqueio/leitura do `xurl`.
