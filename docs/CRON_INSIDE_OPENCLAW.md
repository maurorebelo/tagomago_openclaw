...
1. **Copy twitter-archive-to-nostr** into the container (from the repo on your Mac, or from the host if the repo is there):
   ...
   ```bash
   # From host (repo at /opt/openclaw or similar)
   docker cp /opt/openclaw/scripts/twitter-archive-to-nostr openclaw-b60d-openclaw-1:/data/twitter-archive-to-nostr
   ```

2. **Copy scripts** and make them executable:

   ```bash
   docker cp /opt/openclaw/scripts/cron-twitter-to-nostr-inside-container.sh openclaw-b60d-openclaw-1:/data/scripts/
   docker cp /opt/openclaw/scripts/run-twitter-sync-loop.sh openclaw-b60d-openclaw-1:/data/scripts/
   docker exec openclaw-b60d-openclaw-1 chmod +x /data/scripts/cron-twitter-to-nostr-inside-container.sh /data/scripts/run-twitter-sync-loop.sh
   ```

3. **Ensure tweets.js is available** — e.g. copy from host into container, or mount. If you already have it at `/docker/openclaw-b60d/data/data/tweets.js` on the host:

   ```bash
   docker exec openclaw-b60d-openclaw-1 mkdir -p /data/twitter/data
   docker cp /docker/openclaw-b60d/data/data/tweets.js openclaw-b60d-openclaw-1:/data/twitter/data/tweets.js
   ```

   (You’ll need to refresh this file when you get a new Twitter archive, or use a bind mount so the host path is always visible.)
...
- **`NOSTR_BRIDGE_RELAY`** / **`NOSTR_TARGET_RELAY`** — Bridge and target relay URLs (defaults: bridge.tagomago.me, nostr.tagomago.me).

## Run sync once (manual)

To run the sync once without waiting for the loop:
