...
- **xurl read-only wrapper (audit + block post):** `scripts/xurl-readonly.sh` allows only `xurl whoami` and `xurl timeline ...`; logs every call to `XURL_AUDIT_LOG` (default `/data/.xurl-audit.log`); blocks any other subcommand (e.g. `post`). **Installed on VPS:** `/data/bin/xurl` = wrapper, `/data/bin/xurl-real` = real binary. The sync script `run-live-x-nostr-sync.sh` sets `PATH=/data/bin:$PATH` so sync uses the wrapper. For the **agent** to use the wrapper when it runs `xurl` in the container, add to the container environment (e.g. docker-compose or OpenClaw workspace env): `PATH=/data/bin:$PATH`. Audit log: `cat /data/.xurl-audit.log` in the container.
