...
- **xurl read-only wrapper:** `scripts/xurl-readonly.sh` allows `whoami`, `timeline`, and paths matching `/2/users/[0-9]+/tweets`; logs to `XURL_AUDIT_LOG` (default `/data/.xurl-audit.log`); blocks `post` and other APIs. **Installed on VPS:** `/data/bin/xurl` = wrapper, `/data/bin/xurl-real` = real binary. `run-live-x-nostr-sync.sh` sets `PATH=/data/bin:$PATH`. For the **agent**, set container env `PATH=/data/bin:$PATH`. After updating the script on the host, copy to `/data/bin/xurl` in the container. Audit: `cat /data/.xurl-audit.log`.
